...
#[derive(Debug, Serialize)]
pub struct BoxEntryView { pub id: String, pub box_id: String, pub owned_card_id: Option<String>, pub printing_id: String, pub quantity: i64, pub name: String, pub set_code: String, pub collector_number: String, pub rarity: Option<String>, pub mana_cost: Option<String>, pub mana_value: Option<f64>, pub card_type: Option<String>, pub colors: Vec<String>, pub catalog_only: bool }
...
#[tauri::command]
pub fn list_box_entries(state: State<'_, Mutex<Database>>, box_id: String, query: String) -> Result<Vec<BoxEntryView>, String> { let db=state.lock().map_err(|_| "database lock poisoned".to_string())?; services::list_box_entries(&db,&box_id,&query).map_err(db_error).map(|v|v.into_iter().map(|(e,c)|BoxEntryView{id:e.id,box_id:e.box_id,owned_card_id:e.owned_card_id.clone(),printing_id:e.printing_id,name:c.name,set_code:c.set_code,collector_number:c.collector_number,rarity:c.rarity,mana_cost:c.mana_cost,mana_value:c.mana_value,card_type:c.card_type,colors:c.colors,catalog_only:e.owned_card_id.is_none(),quantity:e.quantity}).collect()) }
#[tauri::command]
