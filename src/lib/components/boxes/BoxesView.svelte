<script lang="ts">
  ...
  async function removeBox(){if(!selected)return;await invoke("delete_box",{id:selected.id});selected=null;entries=[];await loadBoxes()}
  async function rename(){if(!selected||!newName.trim())return;await invoke("update_box",{id:selected.id,name:newName});selected={...selected,name:newName};newName="";await loadBoxes()}
  async function searchCatalog(){if(!addQuery.trim()){results=[];ownedResults=[];return} if(source==="catalog") results=await invoke<Catalog[]>("catalog_search",{request:{query:addQuery,limit:8}}); else ownedResults=await invoke<Entry[]>("search_owned_cards",{request:{query:addQuery}})}
