...
<section class="relative rounded-2xl border border-border bg-panel p-7 max-sm:p-5">
  ...
  {#if loading}<p class="text-sm text-muted">Loading local collection…</p>{:else if displayedCards.length === 0}<div class="grid justify-items-center gap-2 py-12 text-center text-muted"><Icon name="grid" size={28} /><p class="font-semibold text-foreground">{collectionQuery ? "No owned cards match your search." : "Your collection is empty."}</p><small>{collectionQuery ? "Try a different search." : "Use the + button to add your first card."}</small></div>{:else if viewMode === "list"}<div class="overflow-x-auto"><Table.Root><Table.Header><Table.Row><Table.Head>Qty</Table.Head><Table.Head>Card name</Table.Head><Table.Head>Mana cost</Table.Head><Table.Head>Type</Table.Head><Table.Head>Printing</Table.Head><Table.Head><span class="sr-only">Actions</span></Table.Head></Table.Row></Table.Header><Table.Body>{#each displayedCards as card (card.id)}<Table.Row class="cursor-pointer text-[#aab5ce]" tabindex="0" role="button" aria-label={`View details for ${card.name}`} onclick={() => openViewer(card)} onkeydown={(event) => (event.key === "Enter" || event.key === " ") && openViewer(card)}><Table.Cell class="font-bold text-[#f7d889]">{card.quantity}</Table.Cell><Table.Cell><strong class="text-foreground">{card.name}</strong>{#if card.foil}<span class="ml-2 text-[10px] font-bold text-[#d6ae58]">FOIL</span>{/if}<small class="mt-1 block text-[11px] text-[#71809f] md:hidden">{card.condition} · {card.language}</small></Table.Cell><Table.Cell class="mana-cell">{#if manaTokens(card.mana_cost).length}{#each manaTokens(card.mana_cost) as token}<i class="ms ms-cost ms-{token}" aria-label={token}></i>{/each}{:else}—{/if}</Table.Cell><Table.Cell>{card.card_type || "—"}</Table.Cell><Table.Cell>{card.set_code} · {card.collector_number || "—"}</Table.Cell><Table.Cell><Button variant="ghost" size="icon" class="size-8" aria-label={`Edit ${card.name}`} title={`Edit ${card.name}`} onclick={(event) => { event.stopPropagation(); openEdit(card); }}><Icon name="pencil" size={15} /></Button><Button variant="destructive" size="sm" aria-label={removing === card.id ? `Confirm removal of ${card.name}` : `Remove ${card.name}`} onclick={(event) => { event.stopPropagation(); removeCard(card.id); }}><Icon name={removing === card.id ? "x" : "trash"} size={14} />{removing === card.id ? "Confirm" : ""}</Button></Table.Cell></Table.Row>{/each}</Table.Body></Table.Root></div>{:else}<div class="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">{#each displayedCards as card (card.id)}{@const face = card.faces?.[0]}{@const src = imageSource(card)}<article class="overflow-hidden rounded-xl border border-border bg-background shadow-sm"><button class="block w-full text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" aria-label={`View details for ${card.name}`} onclick={() => openViewer(card)}><div class="relative aspect-[5/7] overflow-hidden bg-muted">{#if src && !imageFailedCards.has(card.id)}{#if imageLoading.has(card.id)}<Skeleton class="absolute inset-2 rounded-lg" />{/if}<img class:hidden={imageLoading.has(card.id)} class="h-full w-full object-cover" src={src} alt={`${card.name} card art`} loading="lazy" onload={() => markImageLoaded(card.id)} onerror={() => markImageFailed(card.id)} />{:else}<div class="flex h-full items-center justify-center p-4 text-center text-xs text-muted">{face?.image.status === "stale" ? "Cached image is stale." : "Image unavailable."}</div>{/if}</div><div class="grid gap-1 p-3"><strong class="truncate text-sm text-foreground">{card.name}</strong><span class="text-xs text-muted">Qty {card.quantity} · {card.set_code} · #{card.collector_number}</span><span class="truncate text-xs text-muted">{card.card_type || "Card"}{#if card.foil} · Foil{/if}</span></div></button></article>{/each}</div>{/if}
</section>
